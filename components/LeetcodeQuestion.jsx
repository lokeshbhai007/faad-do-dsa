// components/LeetcodeQuestion.jsx
"use client";
import { useState } from 'react';
import { ArrowRight, CheckCircle, BookMark, MessageSquare, Code, HelpCircle, CodeSquare, BarChart, Bookmark } from 'lucide-react';

export default function LeetcodeQuestion({ question }) {
  ...
  const getDifficultyColor = (difficulty) => {
    switch(difficulty?.toLowerCase()) {
      ...
      default: return 'text-green-500'; // Default to easy
    }
  };
    ...
    <div className="flex flex-col lg:flex-row h-full min-h-screen bg-gray-50">
      ...
      <div className="lg:w-64 bg-white p-4 border-r border-gray-200">
        <div className="flex items-center mb-6">
          <span className="font-mono text-lg font-bold mr-2">{question.questionNumber}.</span>
          <h1 className="text-lg font-medium line-clamp-2">
            {question.originalQuestion.split(' ').slice(0, 5).join(' ')}
            {question.originalQuestion.split(' ').length > 5 ? '...' : ''}
          </h1>
        </div>
        
        <div className="mb-4">
          <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getDifficultyColor(question.difficulty)}`}>
      ...
      <div className="flex-grow">
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="flex">
            <button 
              ...
              className={`px-4 py-3 text-sm font-medium ${activeTab === 'description' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            >
            ...
            <button 
              ...
              className={`px-4 py-3 text-sm font-medium ${activeTab === 'solutions' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            >
              <div className="flex items-center">
            ...
            <button 
              ...
              className={`px-4 py-3 text-sm font-medium ${activeTab === 'hints' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            >
              <div className="flex items-center">
            ...
            <button 
              ...
              className={`px-4 py-3 text-sm font-medium ${activeTab === 'explanation' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            >
              <div className="flex items-center">
        ...
        <div className="p-6 bg-white min-h-screen">
          {activeTab === 'description' && (
            <div>
              <h2 className="text-xl font-bold mb-4">{question.originalQuestion}</h2>
              <div 
                className="prose max-w-none"
                dangerouslySetInnerHTML={{ __html: formatDescription(question.description) }}
              />
          ...
          {activeTab === 'solutions' && (
            <div>
              ...
              {question.solutions && question.solutions.length > 0 && question.solutions[0] !== "No solutions available" ? (
                  ...
                  {question.solutions.map((solution, index) => (
                    <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                      ...
                      <div className="p-4">
                        <div 
                          className="prose max-w-none" 
                          dangerouslySetInnerHTML={{ __html: formatDescription(solution) }}
                        />
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'hints' && (
            <div>
              ...
              <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
                <p className="text-yellow-700">{question.hint || "No hints available for this problem."}</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'explanation' && (
            <div>
              ...
              <div className="bg-blue-50 p-4 rounded-md">
                <p className="text-gray-800 whitespace-pre-line">{question.simplifiedExplanation || "No explanation available."}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="lg:w-64 bg-white p-4 border-l border-gray-200 hidden lg:block">
        <div className="mb-4">
          <h3 className="text-sm font-medium text-gray-700 mb-2">Similar Questions</h3>
